package ca.qc.cstj.bottomnavigation.core.ui.navigation
...
class TopLevelBackStack<T : NavKey>(private val startKey: T) {
    ...
    private val _backStack = mutableStateListOf(startKey)

    var topLevelKey by mutableStateOf(startKey)
        private set

    val backStack: List<T> = _backStack

    val currentKey by derivedStateOf {
        _backStack.last()
    }
    ...
    private fun updateBackStack() {
        ...
        if (topLevelKey == startKey) {
            _backStack.addAll(currentStack)
        } else {
            val startStack = topLevelBackStacks[startKey] ?: emptyList()
            _backStack.addAll(startStack + currentStack)
        }
    }
